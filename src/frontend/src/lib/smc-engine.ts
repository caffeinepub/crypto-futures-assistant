...
export function detectOrderBlocks(candles: OHLCVCandle[]): SMCSignal[] {
  ...
  const avgRange =
    candles.slice(-20).reduce((s, c) => s + candleRange(c), 0) /
  ...
  for (let i = 2; i < candles.length - 2; i++) {
    ...
    if (isBearishCandle(c)) {
      ...
      if (
        moveUp > STRONG_MOVE_THRESHOLD &&
        isBullishCandle(next1) &&
        isBullishCandle(next2)
      ) {
        const bodyRatio = candleBodySize(c) / (avgRange || 1);
        const confidence = Math.min(
          100,
          Math.round(40 + bodyRatio * 30 + moveUp * 1000),
        );
        signals.push({
          type: "OrderBlock",
    ...
    if (isBullishCandle(c)) {
      ...
      if (
        moveDown > STRONG_MOVE_THRESHOLD &&
        isBearishCandle(next1) &&
        isBearishCandle(next2)
      ) {
        const bodyRatio = candleBodySize(c) / (avgRange || 1);
        const confidence = Math.min(
          100,
          Math.round(40 + bodyRatio * 30 + moveDown * 1000),
        );
        signals.push({
          type: "OrderBlock",
          priceLevel: (c.open + c.close) / 2,
...
export function detectBOSAndCHoCH(candles: OHLCVCandle[]): SMCSignal[] {
  ...
  if (highs.length >= 2) {
    ...
    if (lastHigh.price > prevHigh.price && lastCandle.close > lastHigh.price) {
      const breakStrength =
        (lastCandle.close - prevHigh.price) / prevHigh.price;
    ...
    if (lows.length >= 2) {
      ...
      if (lastHigh.price > prevHigh.price && lastCandle.close < lastLow.price) {
        const breakStrength =
          (lastLow.price - lastCandle.close) / lastLow.price;
  ...
  if (lows.length >= 2) {
    ...
    if (highs.length >= 2) {
      ...
      if (lastLow.price < prevLow.price && lastCandle.close > lastHigh.price) {
        const breakStrength =
          (lastCandle.close - lastHigh.price) / lastHigh.price;
        const confidence = Math.min(100, Math.round(60 + breakStrength * 2000));
        signals.push({
          type: "CHoCH",
...
export function detectLiquidityZones(candles: OHLCVCandle[]): SMCSignal[] {
  ...
  for (const swing of recentSwings) {
    ...
    const nearbyCount = recentSwings.filter((s) => {
      return (
        s !== swing && Math.abs(s.price - swing.price) / swing.price < 0.003
      );
    }).length;
...
export function scoreToRecommendation(
  score: number,
): import("./smc-types").RecommendationLabel {
